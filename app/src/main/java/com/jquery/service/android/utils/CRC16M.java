...
public class CRC16M {
    ...
    public static boolean crcModbusIsRight(byte[] data, byte[] crc) {
        if (data.length < 3) {
            return false;
        }
        byte[] res = getCrc(data);
        byte[] calc = new byte[crc.length];
        System.arraycopy(res, res.length - calc.length, calc, 0, calc.length);
        return HexUtil.byteArrayCompare(calc, crc);
    }
    ...
    public static byte[] getCrc(byte[] b) {
        byte[] bt = new byte[2];
        CRC16M crc16 = new CRC16M();
        crc16.update(b, b.length);
        int ri = crc16.getValue();
        bt[0] = (byte) (0xff & ri);
        bt[1] = (byte) ((0xff00 & ri) >> 8);
        return bt;
    }
