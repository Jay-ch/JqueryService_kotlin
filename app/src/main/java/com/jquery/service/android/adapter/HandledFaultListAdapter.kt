 ...
 */
class HandledFaultListAdapter : CommRecyclerAdapter<HandledFaultListEntity> {
    ...
    override fun getLayoutResId(item: HandledFaultListEntity, position: Int): Int {
        return if (showList) {
            R.layout.item_handled_fault_list_layout
        } else {
            R.layout.item_handled_fault_layout
        }
    }

    @SuppressLint("StringFormatInvalid")
    override fun onUpdate(helper: BaseAdapterHelper, item: HandledFaultListEntity, position: Int) {
        /* if (item.photo != null) {
