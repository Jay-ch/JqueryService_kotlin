...
import com.gyf.barlibrary.OSUtils.isFlymeOS
import com.gyf.barlibrary.OSUtils.isMIUI
...
//            }
     ...
     */
    fun setStatusViewAttr( activity: Activity?,view: View?,@ColorInt color: Int) {
     ...
     * @return
     */
    fun setStatusBarMode(activity: Activity, isTextDark: Boolean, @ColorInt colorId: Int,view: View?) {
        if (!isTextDark) {
            //文字、图标颜色不变，只修改状态栏颜色
            setStatusViewAttr(activity, view,colorId)
        } else {
            //修改状态栏颜色和文字图标颜色
            setStatusViewAttr(activity, view,colorId)
            //4.4以上才可以改文字图标颜色
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                if (isMIUI()) {
                    //小米MIUI系统
                    setMIUIStatusBarDarkIcon(activity, isTextDark)
                } else if (isFlymeOS()) {
                    //魅族flyme系统
                    setMeizuStatusBarDarkIcon(activity, isTextDark)
